...
class ArtifactSetService(ModelService):
    ...
    async def _pre_save(cls, schema: CreateSchema | UpdateSchema, exclude_none: bool = True) -> dict[str, Any]:
        two_piece_bonus = await ArtifactSetBonusService.get_or_create(
            schema.two_piece_bonus
        )
        four_piece_bonus = await ArtifactSetBonusService.get_or_create(
            schema.four_piece_bonus
        )
...
class ArtifactService(ModelService):
    ...
    async def _pre_save(cls, schema: CreateSchema | UpdateSchema, exclude_none: bool = True) -> dict[str, Any]:
        ...
        if (schema.main_stat.stat not in
            ALLOWED_ARTIFACT_MAIN_STATS_FOR_TYPE[schema.artifact_type]):
            raise HTTPException(
                status_code=400,
